...
1. Find an optimum scheduling for the following job assignment problem with the given cost table using an appropriate combinatorial optimization approach:
...
2. Find an optimum scheduling for the following job assignment problem with the given cost table using an appropriate combinatorial optimization approach:
...
3. Find the Minimum Spanning Tree (MST) and its total cost for the following weighted graph with vertices {A,B,C,D} and the edge and edge weights as: (A, B)=10, (A, C)=6, (A, D)=5, (B, D)=5, (B, E)=3, (C, D)=4.

4. A project manager has to assign 4 tasks to 4 employees to minimize the total project run cost. An effect matrix is formulated for the cost per employee and per-employee efficiency, as given below. Solve this job allocation problem using a suitable combinatorial optimization technique.
...
5. Find the shortest path in the following graph with vertices {A,B,C ,D,E} having edges : (A, B)=4, (A,C)=8, (B,D)=6, (C, E)=2, (D, E)= 4. Assume A is the start node, and E is the end node.
2. Find the Minimum Spanning Tree (MST) and its total cost for the following weighted graph with vertices {A,B,C,D,E} and the edge and edge weights as: (A, B)=4, (A, C)=2, (B, C)=3, (B, D)=2, (B, E)=3, (C, D)=4, (C, E)=5, and (D, E)=1.

6. Segment the following medical image pixel matrix into foreground and background using the minimal spanning tree algorithm (use a threshold of 50 for merging sub-trees).

|100| 105| 200|
|98| 102| 195|
|95| 100| 105|

7. Using Dijkstra's algorithm, find the shortest path from vertex A to vertex E in the following directed graph with edge weights: (A, B)=1, (A, C)=4, (B, C)=2, (B, D)=5, (C, D)=1, (D, E)=3.
8. A project manager has to assign 4 tasks to 4 employees to minimize the total project run cost. An effect matrix is formulated for the cost per employee and per-employee efficiency, as given below. Solve this job allocation problem using a suitable combinatorial optimization technique.
...
9. A delivery company has to deliver packages to 5 different locations. The distance between the locations is given in the table below. Find the optimal route for the delivery truck using the Traveling Salesman Problem (TSP) approach.  
...
10. Explain the Dijkstra's algorithm to find the shortest path in a graph optimization and find the shortest path in the following graph with vertices {A,B,C ,D} having edges : (A, B)=10, (A,C)=6, (A,D)=5, (B, D)=5, (C, D)= 4.
11. A salesman has to visit five cities: A, B, C, D, and E. The distances between the five cities are given in the table below. If the salesman starts from city A and has to return to city A, which route should he take to minimize the total distance travelled?
